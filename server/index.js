...
/**
 * Setup wizard - Create initial admin account
 * POST /api/setup
 */
app.post('/api/setup', async (req, res) => {
    try {
        ...
        // Create admin user
        const adminUser = await authManager.register(admin.username, admin.password, 'admin');

        // Save settings to config file
        const configPath = path.join(__dirname, '..', 'data', 'config.json');
        const config = {
            defaultMemory: settings.defaultMemory || '2G',
